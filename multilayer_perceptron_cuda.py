...
class MultilayerPerceptronModel(nn.Module):
    ...
    def tensor_load(self, inputs, batchsize):
        ...
        input_tensor = torch.stack([item[0] for item in inputs]).to(device)
        label_tensor = torch.tensor([(item[1]) for item in inputs]).to(device)
    ...
    def predict(self, model_input: torch.Tensor):
        ...
        self.eval()
        model_input = model_input.long().to(device)
...
if __name__ == "__main__":
    ...
    model = MultilayerPerceptronModel(num_of_classes, vocab_size,  embedding_dim= 128).to(device)
